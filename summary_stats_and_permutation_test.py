...
columns_to_drop = ['charity_age_ord', 'ncvo_size_categories', 'charity_size',
  'region_of_op', 'date_of_registration',     'mean_gross_income', 'area_of_operation', 'years_not_active']
df = df.drop(columns=columns_to_drop)

df['Row_Sum'] = df.iloc[:, 2:].sum(axis=1)
...
# crosstabs_df.to_csv(f'{DATA_DIR}/outputs/table_1.csv', index=False)
